...
export class ProductService {
    async getProductData(productName) {
        ...
        if (existingProduct.rows.length > 0) {
            ...
            await query('UPDATE products SET overall_real_score = $1, hype_score = $2, reality_score = $3, last_updated = NOW() WHERE id = $4', [analysis.reality_score, analysis.hype_score, analysis.reality_score, productId]);
            // Clear old red flags
            ...
            const insertResult = await query('INSERT INTO products (name, overall_real_score, hype_score, reality_score) VALUES ($1, $2, $3, $4) RETURNING id', [productName, analysis.reality_score, analysis.hype_score, analysis.reality_score]);
            productId = insertResult.rows[0].id;
        }
        // Store red flags
        for (const flag of analysis.red_flags) {
            await query('INSERT INTO red_flags (product_id, issue_summary, severity) VALUES ($1, $2, $3)', [productId, flag.issue, flag.severity]);
        }
        // Store a few review snippets
        for (const review of reviews.slice(0, 10)) {
            await query('INSERT INTO cached_reviews (product_id, source_url, content_snippet) VALUES ($1, $2, $3)', [productId, review.source_url, review.content.slice(0, 500)]);
        }
